...
export const ProgressBarLink = forwardRef<
  HTMLAnchorElement, // Specify the element type for the ref
  ComponentProps<typeof Link> // Extend the props from Link component
>(function ProgressBarLink({ href, children, ...rest }:ComponentProps<typeof Link>, ref: any) {
  ...
  return (
    <Link
      href={href}
      ref={ref} // Forward the ref to the underlying Link component
      onClick={(e) => {
        e.preventDefault();
        progress.start(); // Start the progress bar when link is clicked

        startTransition(() => {
          router.push(href.toString()); // Navigate to the new route
          progress.done(); // Complete the progress bar
        });
      }}
      {...rest}
    >
