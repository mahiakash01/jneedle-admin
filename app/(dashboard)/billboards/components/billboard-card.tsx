...
const BillboardCard = ({
  ...
  const onDelete = async (item: BillboardProps) => {
  ...
  console.log("item: ", billboardItem.image);
  return (
    <Card className="w-[200px]" key={billboardItem.$id}>
      <div className="w-full h-[150px] flex justify-center items-center">
        <img
          src={billboardItem.image.previewUrl}
          alt=""
          width={50}
          height={50}
      ...
      <div className="flex justify-between items-center p-4">
        <p className="capitalize">{billboardItem.title}</p>
        <Button
          variant={"ghost"}
          className="p-2"
          disabled={deleteBillboard}
          onClick={() => onDelete(billboardItem)}
        >
          {deleteBillboard ? (
            <LoadingSpinner size={18} className="text-red-700" />
          ) : (
